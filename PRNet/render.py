    ...
    for i in range(triangles.shape[1]):
        ...
        for u in range(umin, umax+1):
            for v in range(vmin, vmax+1):
                if not isPointInTri([u,v], vertices[:2, tri]):
                    continue
                w0, w1, w2 = get_point_weight([u, v], vertices[:2, tri])
                point_depth = w0 * vertices[2, tri[0]] + w1 * vertices[2, tri[1]] + w2 * vertices[2, tri[2]]
                if point_depth > depth_buffer[v, u]:
                    depth_buffer[v, u] = point_depth
                    image[v, u, :] = w0*colors[:, tri[0]] + w1*colors[:, tri[0]] + w2*colors[:, tri[0]]
    return image
