...
def get_rects_image(image,rects_list,landmarks,resize_height=None, resize_width=None):
    rect_images = []
    for (i, rect) in enumerate(rects_list):
        rotated = get_align_image(image, landmarks[i])

        roi=get_rect_image(rotated, rects_list[i])
        roi=resize_image(roi, resize_height, resize_width)
        rect_images.append(roi)
    return rect_images

def get_align_image(image, landmark):

    left_point = (float(landmark[0]), float(landmark[1]))

    right_point = (float(landmark[2]), float(landmark[3]))

    s_fa = left_point[1] - right_point[1]

    level_point = (left_point[0], right_point[1])

    angle = cos_angle(left_point, right_point, level_point)


    center = ((float(landmark[0]) + float(landmark[2])) / 2.0, (float(landmark[1] +float(landmark[3])) / 2.0))

    rotated = rotate_bound(image,angle,center)

    return rotated



def get_bboxes_image(image,bboxes_list,landmarks,resize_height=None, resize_width=None):
    rects_list=bboxes2rects(bboxes_list)

    rect_images = get_rects_image(image,rects_list,landmarks,resize_height, resize_width)
    return rect_images
...
def show_image_bboxes_text(title, rgb_image, boxes, boxes_name):
    ...
    for name ,box in zip(boxes_name,boxes):
        ...
        cv2.rectangle(rgb_image, (box[0],box[1]),(box[2],box[3]), (0, 255, 0), 2, 8, 0)
        cv2.putText(rgb_image,name, (box[0],box[1]), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), thickness=2)
    # cv2.imshow(title, bgr_image)
    # cv2.waitKey(0)
    # rgb_image = cv2.cvtColorshow_image_bboxes_text(bgr_image, cv2.COLOR_BGR2RGB)
...
def cos_angle(a,b,c,isAngle = True):
    dx1 = b[0] - a[0]
    dy1 = b[1] - a[1]
    dx2 = c[0] - a[0]
    dy2 = c[1] - b[1]
...
def get_face_mask(image_size, face_landmarks):
    ...
    points = []
    kpt = np.round(face_landmarks).astype(np.int32)
    for i in range(kpt.shape[0]):
        st = kpt[i, :2]
        points.append([st[0], st[1]])
